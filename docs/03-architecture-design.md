# 系统架构设计文档

> 项目：Persona Knowledge Base V2
> 文档版本：v1.0
> 创建日期：2026-01-13
> 阶段：架构设计

---

## 📌 架构概述

### 架构风格
> **模块化架构（Modular Architecture）**

### 设计原则
1. **模块独立** - 各模块职责清晰，低耦合高内聚
2. **分层清晰** - UI层、业务层、数据层分离
3. **可扩展性** - 便于添加新功能和模块
4. **可测试性** - 模块化便于单元测试

---

## 🏗️ 系统整体架构

### 架构图

```
┌─────────────────────────────────────────────────┐
│         Persona Knowledge Base V2               │
│              (Electron 桌面应用)                 │
├─────────────────────────────────────────────────┤
│                                                 │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐      │
│  │  UI 模块  │  │  核心模块  │  │ AI 模块   │      │
│  │  (React)  │  │ (Business)│  │(LangChain)│      │
│  │          │  │          │  │          │      │
│  │ - 主界面  │  │ - 笔记管理│  │ - LLM调用│      │
│  │ - 检索界面│  │ - 文件IO  │  │ - Prompt │      │
│  │ - 设置界面│  │ - 状态管理│  │ - Chain  │      │
│  │ - 对话界面│  │ - 结构分析│  │ - 记忆   │      │
│  └────┬─────┘  └────┬─────┘  └────┬─────┘      │
│       │             │             │           │
│       └─────────────┴─────────────┘           │
│                     │                         │
│              ┌──────┴──────┐                  │
│              │  数据层      │                  │
│              │ (Data Layer)│                  │
│              │             │                  │
│              │ - 本地文件  │ ←─────────────┐  │
│              │ - 配置管理  │               │  │
│              │ - 向量数据库│               │  │
│              └─────────────┘               │  │
│                     │                        │  │
└─────────────────────┼────────────────────────┘  │
                      │                           │
         ┌────────────┴────────────┐            │
         ↓                         ↓            │
┌─────────────────┐      ┌─────────────────┐     │
│  本地文件系统    │      │  DeepSeek API   │     │
│  (.md 笔记)     │      │  (LLM 服务)      │     │
└─────────────────┘      └─────────────────┘     │
                                                   │
                   外部依赖                        │
```

---

## 📦 模块详细设计

### 1. UI 模块 (UI Layer)

**职责**：负责用户界面展示和交互

**子模块**：

| 子模块 | 组件 | 功能 |
|--------|------|------|
| **主界面** | Dashboard | 应用主入口、快捷功能入口 |
| **检索界面** | SearchPanel | F1：智能检索功能界面 |
| **整理界面** | OrganizePanel | F2：碎片信息整理界面 |
| **对话界面** | ChatPanel | F3：AI问答对话界面 |
| **建议界面** | SuggestPanel | F4：优化建议展示界面 |
| **设置界面** | SettingsPanel | 应用设置、API配置 |

**技术栈**：React + TypeScript + shadcn/ui

---

### 2. 核心模块 (Core Layer)

**职责**：业务逻辑处理和数据管理

**子模块**：

| 子模块 | 功能 |
|--------|------|
| **笔记管理器** (NoteManager) | 读取、解析、保存 Obsidian 笔记 |
| **文件IO** (FileService) | 文件读写、目录操作 |
| **状态管理** (StoreManager) | 应用状态管理（Zustand/Redux） |
| **结构分析器** (StructureAnalyzer) | 分析知识库结构，生成优化建议 |
| **向量化服务** (EmbeddingService) | 文本向量化，用于智能检索 |

**技术栈**：TypeScript + Node.js

---

### 3. AI 模块 (AI Layer)

**职责**：AI 能力封装和 LangChain 集成

**子模块**：

| 子模块 | 功能 |
|--------|------|
| **LLM 客户端** (LLMClient) | 封装 DeepSeek API 调用 |
| **Prompt 管理器** (PromptManager) | 管理提示词模板 |
| **Chain 编排器** (ChainBuilder) | 构建和执行 LangChain Chain |
| **记忆管理** (MemoryManager) | 管理对话记忆和历史 |
| **检索器** (Retriever) | 实现智能检索（RAG） |

**技术栈**：LangChain.js + DeepSeek API

---

### 4. 数据层 (Data Layer)

**职责**：数据持久化和存储

**子模块**：

| 子模块 | 功能 |
|--------|------|
| **本地存储** (LocalStorage) | 用户配置、偏好设置 |
| **文件存储** (FileStorage) | Obsidian 笔记目录管理 |
| **向量数据库** (VectorDB) | 存储文本向量（可选：ChromaDB、Pinecone） |

**技术栈**：Node.js fs + JSON + 向量数据库（待定）

---

## 🎯 功能模块映射

### 功能与模块关系图

| 功能ID | 功能名称 | 涉及模块 | 核心组件 | 数据流 |
|--------|---------|---------|---------|--------|
| **F1** | 智能检索 | UI + 核心 + AI | SearchPanel, Retriever, LLMClient | 用户查询 → 向量检索 → LLM重排 → 结果展示 |
| **F2** | 碎片信息整理 | UI + 核心 + AI | OrganizePanel, NoteManager, Chain | 碎片文本 → LLM总结 → 结构化文档 → 保存 |
| **F3** | 问答经验归档 | UI + 核心 + AI | ChatPanel, LLMClient, FileService | 对话记录 → LLM总结 → 自动归档 |
| **F4** | 知识库优化建议 | UI + 核心 + AI | SuggestPanel, StructureAnalyzer, LLMClient | 知识库扫描 → 结构分析 → LLM建议 |
| **F5** | 用户画像上下文 | AI + 数据 | MemoryManager, VectorDB | 知识库加载 → 向量化 → 上下文注入 |

---

## 🔄 数据流设计

### 场景1：智能检索 (F1)

```
用户输入自然语言查询
    ↓
┌──────────────────┐
│  SearchPanel     │
│  (UI 模块)       │
└────────┬─────────┘
         │
         ↓
┌──────────────────┐
│  Retriever       │
│  (AI 模块)       │ ←───── VectorDB (向量数据库)
└────────┬─────────┘
         │ 向量检索
         ↓
┌──────────────────┐
│  LLMClient       │
│  (AI 模块)       │ ←───── DeepSeek API
└────────┬─────────┘
         │ 重排序
         ↓
┌──────────────────┐
│  SearchPanel    │
│  (UI 模块)       │
└────────┬─────────┘
         │
         ↓
    展示检索结果
```

---

### 场景2：碎片信息整理 (F2)

```
用户粘贴碎片化文本
    ↓
┌──────────────────┐
│  OrganizePanel   │
│  (UI 模块)       │
└────────┬─────────┘
         │
         ↓
┌──────────────────┐
│  ChainBuilder    │
│  (AI 模块)       │ ←───── PromptManager
└────────┬─────────┘
         │
         ↓
┌──────────────────┐
│  LLMClient       │
│  (AI 模块)       │ ←───── DeepSeek API
└────────┬─────────┘
         │
         ↓
┌──────────────────┐
│  NoteManager     │
│  (核心模块)       │
└────────┬─────────┘
         │
         ↓
┌──────────────────┐
│  FileStorage     │
│  (数据层)         │
└────────┬─────────┘
         │
         ↓
    保存结构化文档
```

---

### 场景3：问答经验归档 (F3)

```
用户与AI对话
    ↓
┌──────────────────┐
│  ChatPanel       │
│  (UI 模块)       │
└────────┬─────────┘
         │
         ↓
┌──────────────────┐
│  MemoryManager   │
│  (AI 模块)       │
└────────┬─────────┘
         │ 记录对话
         ↓
┌──────────────────┐
│  LLMClient       │
│  (AI 模块)       │ ←───── DeepSeek API
└────────┬─────────┘
         │ 总结经验
         ↓
┌──────────────────┐
│  StructureAnalyzer│
│  (核心模块)       │
└────────┬─────────┘
         │ 分析分类
         ↓
┌──────────────────┐
│  FileStorage     │
│  (数据层)         │
└────────┬─────────┘
         │
         ↓
    自动归档到对应位置
```

---

## 📁 项目目录结构

```
persona-knowledge-base-v2/
├── src/
│   ├── main/                    # Electron 主进程
│   │   ├── index.ts            # 主进程入口
│   │   ├── ipc/                # IPC 通信
│   │   └── services/           # 系统服务
│   │
│   ├── renderer/               # 渲染进程（React）
│   │   ├── App.tsx             # 根组件
│   │   ├── components/         # UI 组件
│   │   │   ├── Dashboard/
│   │   │   ├── SearchPanel/
│   │   │   ├── OrganizePanel/
│   │   │   ├── ChatPanel/
│   │   │   ├── SuggestPanel/
│   │   │   └── Settings/
│   │   ├── store/              # 状态管理
│   │   ├── hooks/              # 自定义 Hooks
│   │   └── utils/              # 工具函数
│   │
│   ├── core/                   # 核心业务模块
│   │   ├── NoteManager.ts
│   │   ├── FileService.ts
│   │   ├── StructureAnalyzer.ts
│   │   └── EmbeddingService.ts
│   │
│   ├── ai/                     # AI 模块
│   │   ├── LLMClient.ts
│   │   ├── PromptManager.ts
│   │   ├── ChainBuilder.ts
│   │   ├── MemoryManager.ts
│   │   └── Retriever.ts
│   │
│   └── shared/                 # 共享代码
│       ├── types/              # TypeScript 类型
│       └── constants/          # 常量
│
├── docs/                       # 文档
│   ├── 01-requirements-analysis.md
│   ├── 02-tech-selection.md
│   ├── 03-architecture-design.md
│   └── ...
│
├── package.json
├── tsconfig.json
├── vite.config.ts
└── electron-builder.yml        # 打包配置
```

---

## 🔌 接口设计

### IPC 通信接口

Electron 主进程与渲染进程之间的通信接口：

| 接口名称 | 类型 | 参数 | 返回值 | 用途 |
|---------|------|------|--------|------|
| `getNoteList` | invoke | `{ path: string }` | `Note[]` | 获取笔记列表 |
| `readNote` | invoke | `{ filePath: string }` | `string` | 读取笔记内容 |
| `saveNote` | invoke | `{ filePath: string, content: string }` | `void` | 保存笔记 |
| `selectDirectory` | invoke | - | `string` | 选择笔记目录 |
| `getConfig` | invoke | - | `Config` | 获取配置 |
| `setConfig` | invoke | `{ config: Config }` | `void` | 保存配置 |

---

## 🗃️ 数据模型

### 笔记 (Note)

```typescript
interface Note {
  id: string;              // 笔记唯一ID
  title: string;           // 笔记标题
  path: string;            // 文件路径
  content: string;         // 笔记内容
  tags: string[];          // 标签
  createdAt: Date;         // 创建时间
  updatedAt: Date;         // 更新时间
  category?: string;       // 分类
}
```

### 配置 (Config)

```typescript
interface Config {
  obsidianPath: string;        // Obsidian 笔记目录
  deepseekApiKey: string;      // DeepSeek API Key
  deepseekModel: string;       // 模型名称
  theme: 'light' | 'dark';     // 主题
  language: 'zh' | 'en';       // 语言
}
```

---

## 🔒 安全考虑

### API Key 安全
- API Key 加密存储
- 不提交到 Git（在 .gitignore）
- 用户手动输入

### 文件操作安全
- 验证文件路径
- 限制访问范围
- 备份机制

---

## 📈 扩展性设计

### 新增功能扩展点

1. **新增 AI 功能**：在 AI 模块添加新的 Chain
2. **新增 UI 界面**：在 UI 模块添加新组件
3. **新增数据源**：在数据层添加新存储
4. **新增 LLM 服务**：在 LLMClient 添加适配器

---

## 🧪 测试策略

### 测试层次

1. **单元测试**：各模块独立测试
2. **集成测试**：模块间通信测试
3. **E2E 测试**：完整功能流程测试

### 测试工具

- Vitest（单元测试）
- Playwright（E2E 测试）

---

**文档状态**：架构设计完成 ✅
**下一步**：初始化项目并开始编码
